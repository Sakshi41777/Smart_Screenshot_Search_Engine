...
// allow renderer to discover the current backend address (e.g. if it was set via
// environment or chosen dynamically during start-up)
ipcMain.handle("get-backend-url", () => BACKEND_BASE_URL);


ipcMain.handle("select-image", async () => {
  const pictures = app.getPath("pictures");
  const defaultScreenshots = path.join(pictures, "Screenshots");
  const defaultPath = fs.existsSync(defaultScreenshots) ? defaultScreenshots : pictures;
...
ipcMain.handle("get-default-index-folders", async () => {
  ...
  const deduped = [...new Set(candidates.map((p) => path.resolve(p)))];
...
ipcMain.handle("get-background-index-settings", async () => {
  ...
  const status = {
    ...
    watched: watcherState.watchers.size,
  };
  return { ...settings, status };
});

ipcMain.handle("set-background-index-settings", async (_, payload) => {
  const current = watcherState.settings || loadBackgroundSettings();
  const next = {
    consentGiven:
      typeof payload?.consentGiven === "boolean"
    ...
    enabled:
      typeof payload?.enabled === "boolean"
        ...
        : current.enabled,
    strictPrivacy:
      typeof payload?.strictPrivacy === "boolean"
        ? payload.strictPrivacy
        : current.strictPrivacy,
    keepRunningInTray:
      typeof payload?.keepRunningInTray === "boolean"
        ? payload.keepRunningInTray
        : current.keepRunningInTray,
    runAtStartup:
      typeof payload?.runAtStartup === "boolean"
        ? payload.runAtStartup
        : current.runAtStartup,
    useSpecificFolders:
      typeof payload?.useSpecificFolders === "boolean"
        ? payload.useSpecificFolders
        : current.useSpecificFolders,
    folders: Array.isArray(payload?.folders) ? payload.folders : current.folders,
  };
  const saved = saveBackgroundSettings(next);
  await startBackgroundWatchers(saved);
  if (saved?.consentGiven && saved?.enabled && saved?.keepRunningInTray) {
    ensureTray();
  } else if (tray) {
    try {
      tray.destroy();
...
ipcMain.handle("merge-background-index-folders", async (_, folders) => {
  const current = watcherState.settings || loadBackgroundSettings();
  const mergedFolders = normalizeFolderList([
    ...(current.folders || []),
...
ipcMain.handle("open-path", async (_, targetPath) => {
  if (!targetPath || typeof targetPath !== "string") return "invalid_path";
  if (!fs.existsSync(targetPath)) return "path_not_found";
...
ipcMain.handle("show-item-in-folder", async (_, targetPath) => {
  ...
  if (process.platform === "win32") {
    return new Promise((resolve) => {
      ...
      execFile("explorer.exe", [`/select,${winPath}`], (err) => {
        if (!err) {
          resolve(true);
          return;
        }

        // Fallback for environments where explorer select fails.
        try {
...
ipcMain.handle("copy-text", async (_, textValue) => {
  try {
